...
int main() 
{
	...
	constexpr double maxBeta = 1.8;
	constexpr double minTheta = 0.8;
	constexpr double maxTheta = 1.5;
	constexpr double maxTP = 0.05;
	
	int n_tot = 10000;
	for (int n = 0; n < n_tot; ++n)
	{		
		std::filesystem::create_directory("2020_05_25_threshold");
		std::filesystem::create_directory("2020_05_25_threshold/" + std::to_string(n));
		std::ofstream pf("2020_05_25_threshold/" + std::to_string(n) + "/parameters.h");
		...
		pf << "constexpr double thresF = " << minTheta + ru() * (maxTheta - minTheta) << "; // Threshold for F expression, higher => female. Below or above 1 (ALWAYS well below 2)." << std::endl;
		...
		pf << "constexpr double FE_diff = " << 1.0 + ru() * (maxBeta - 1.0) << "; // Fold difference in F expression between minimal and maximal temperature." << std::endl;
