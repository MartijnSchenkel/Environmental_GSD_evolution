...
int main() 
{
	...
	{		
		...
		double thetaM_uncorrected = ru();
		double thetaF_uncorrected = thetaM_uncorrected + ru() * (1.0 - thetaM_uncorrected);
		double thetaM_corrected = thetaM_uncorrected * maxTheta;
		double thetaF_corrected = thetaF_uncorrected * maxTheta;

		std::filesystem::create_directory("2021_06_22_thresholds");
		std::filesystem::create_directory("2021_06_22_thresholds/" + std::to_string(n));
		std::ofstream pf("2021_06_22_threshold/" + std::to_string(n) + "/parameters.h");
		...
		pf << "constexpr double survivalYY = 1.0; // proportion of YY individuals that survive" << std::endl << std::endl;
		...
		pf << "constexpr double sSAM = 0; // fitness effect of SA in males" << std::endl;
		...
		pf << "constexpr double thresF = " << thetaF_corrected << "; // Threshold for F expression, higher => female. Below or above 1 (ALWAYS well below 2)." << std::endl;
		pf << "constexpr double thresM = " << thetaM_corrected << "; // Threshold for F expression, lower => male; if thresM < expression < thresF then => intersex." << std::endl;
		...
		pf << "constexpr double FE_diff = " << 1.0 << "; // Fold difference in F expression between minimal and maximal temperature." << std::endl;
