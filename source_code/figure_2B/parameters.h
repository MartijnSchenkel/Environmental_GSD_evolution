...
constexpr long seed_generate = 1168593893;

// Controls for which procedures affect SD in what ways
constexpr bool temperature_F_expression = true; // F expression temperature-sensitive
...
// Runtime parameters
constexpr int burnin = 5000; // Number of generations without temperature differences in model
constexpr int warmup_time = 0; // Number of generations during which temperature increases
constexpr int runTime = 0; // Number of generations to run the model after temperature is increased
constexpr int genskip = 5; // Number of generations between writing output

// Population parameters
constexpr int PopSize = 10000; // Population size per deme
...
constexpr double survivalYY = 1.0; // proportion of YY individuals that survive

constexpr double dSAF = 0.5; // dominance of SA effect in females
constexpr double dSAM = 0.5; // dominance of SA effect in males
constexpr double sSAM = 0; // fitness effect of SA in males
...
constexpr double thresF = 0.788795; // Threshold for F expression, higher => female. Below or above 1 (ALWAYS well below 2).
constexpr double thresM = 0.398525; // Threshold for F expression, lower => male; if thresM < expression < thresF then => intersex.
...
constexpr double muActivAM = 0.0269717; // Rate at which AM becomes expressed (de novo M-factor evolution)
constexpr double transpositionRate = muActivAM; // Rate at which M-factors will attempt to transpose to different genomic location

// F expression ~ temperature parameters
constexpr double FE_diff = 1; // Fold difference in F expression between minimal and maximal temperature.
