...
void Individual::mutateSD_old(const double Temperature)
{
	// Mutate SD genes
    ...
    // Transposition of M-factors ** you have to explain this sometime **
	if (transpositionRate > 0)
	{
		double TR_realized = transpositionRate;
		...
		if (temperature_M_transposition)
		{
			...
			TR_realized = TR_realized * TR_mult;
		}

		// Transpose M-factor activities
		// Determine if M-factors will attempt to transpose (only active M-factors can transpose)
		std::array<bool, 4> willTranspose = { false };
		if (YM[0]) if (ru() < TR_realized) willTranspose[0] = true;
		if (YM[1]) if (ru() < TR_realized) willTranspose[1] = true;
		if (AM[0]) if (ru() < TR_realized) willTranspose[2] = true;
		if (AM[1]) if (ru() < TR_realized) willTranspose[3] = true;

		// Determine to which linkage group and which allele M will transpose
		std::array<bool, 2 * 2> targetLinkageGroup = { false };
		std::array<bool, 2 * 2> targetAllele = { false };

		// Array with all M-factors in their current position
		std::array<std::optional<M_factor>, 2 * 2> Ms = { YM[0], YM[1], AM[0], AM[1] };

		std::array<std::optional<M_factor>, 2> newYM = YM;
		std::array<std::optional<M_factor>, 2> newAM = AM;

		// Randomize order in which M-factors transpose. 
        std::vector<size_t> order = { 0, 1, 2, 3 };
		std::shuffle(order.begin(), order.end(), rng);
		/* Only relevant when more than 1 M-factors would transpose and both would transpose to the exact same genomic location.
		Still, I think it's better to randomize the order than to systematically allow e.g. AM to transpose later and possibly
		bias the effect of transposing AMs (e.g. when AM transposes last, the effects of YM transposition may be reduced because
		transposed YMs may be masked by transposed AMs */

		// Sample target location for transposition and integrate M-factor in that position
        for (size_t i = 0; i < 2 * 2; ++i)
		{
			if (willTranspose[order[i]])
			{		
				targetLinkageGroup[order[i]] = r2(); // 0 = Y, 1 = A
				targetAllele[order[i]] = r2(); // 0 = maternal allele, 1 = paternal allele

				// Integrate transposing M-factor into sample linkage group + allele
				if (!targetLinkageGroup[order[i]])
				{
					// Check if M can land on X; if not, check if target is X or Y
					if (!M_on_X)
					{
						// Check if Y; if yes, allow M to integrate
						if (YChromosomes[targetAllele[order[i]]])
						{
							newYM[targetAllele[order[i]]] = Ms[order[i]];
						}
					} else {
						// If M can be on X, just integrate it without checking if its X or Y.
						newYM[targetAllele[order[i]]] = Ms[order[i]];
					}					
				}
				else {
					newAM[targetAllele[order[i]]] = Ms[order[i]];
...
void Individual::mutateSD(const double Temperature)
{
	...
	if (deNovoAM)
	{
		/*// Old version where AM can only 'become active' if they have never been active before
		if (!AM[0]) if (ru() < muActivAM) AM[0] = rnorm(baseExprM, baseExprMSD);
		if (!AM[1]) if (ru() < muActivAM) AM[1] = rnorm(baseExprM, baseExprMSD);
		*/
		
		// New version where also expressed Ms can evolve 
		if (AM[0]->getExpression(0.0) == 0) if (ru() < muActivAM) AM[0] = rnorm(baseExprM, baseExprMSD);
		if (AM[1]->getExpression(0.0) == 0) if (ru() < muActivAM) AM[1] = rnorm(baseExprM, baseExprMSD);
	} 
...
{
	// Check which Ms are wildtype (= not carrying M-factor, i.e. NEVER have evolved an expressed M)
	bool wtYM[2], wtAM[2];
	for (int i = 0; i < 2; ++i)
	{
		if (YM[i]) { wtYM[i] = false; }
		else { wtYM[i] = true; }

		if (AM[i]) { wtAM[i] = false; }
		else { wtAM[i] = true; }
	}

    const int sx = static_cast<std::underlying_type<Sex>::type>(sex);
	data << Time << "\t" << Deme << "\t" << sx << "\tF\tSensitivity\t" << F[0].getSensitivity(0.0) << "\t" << F[0].getCanMutateSens() << "\t" << 1 << std::endl;
    data << Time << "\t" << Deme << "\t" << sx << "\tF\tSensitivity\t" << F[1].getSensitivity(0.0) << "\t" << F[1].getCanMutateSens() << "\t" << 1 << std::endl;

    data << Time << "\t" << Deme << "\t" << sx << "\tF\tExpression\t" << F[0].getExpression(0.0) << "\t" << F[0].getCanMutateExpr() << "\t" << 1 << std::endl;
    data << Time << "\t" << Deme << "\t" << sx << "\tF\tExpression\t" << F[1].getExpression(0.0) << "\t" << F[1].getCanMutateExpr() << "\t" << 1 << std::endl;
	
	data << Time << "\t" << Deme << "\t" << sx << "\tYM\tExpression\t" << YM[0]->getExpression(0.0) << "\t" << YM[0]->getCanMutateExpr() << "\t" << wtYM[0] << std::endl;
    data << Time << "\t" << Deme << "\t" << sx << "\tYM\tExpression\t" << YM[1]->getExpression(0.0) << "\t" << YM[1]->getCanMutateExpr() << "\t" << wtYM[1] << std::endl;
	
    data << Time << "\t" << Deme << "\t" << sx << "\tAM\tExpression\t" << AM[0]->getExpression(0.0) << "\t" << AM[0]->getCanMutateExpr() << "\t" << wtAM[0] << std::endl;
    data << Time << "\t" << Deme << "\t" << sx << "\tAM\tExpression\t" << AM[1]->getExpression(0.0) << "\t" << AM[1]->getCanMutateExpr() << "\t" << wtAM[1] << std::endl;

    data << Time << "\t" << Deme << "\t" << sx << "\tSexChromosome\tType\t" << YChromosomes[0] << "\t" << YM[0]->getCanMutateExpr() << "\t" << wtYM[0] << std::endl;
    data << Time << "\t" << Deme << "\t" << sx << "\tSexChromosome\tType\t" << YChromosomes[1] << "\t" << YM[1]->getCanMutateExpr() << "\t" << wtYM[1] << std::endl;
}
