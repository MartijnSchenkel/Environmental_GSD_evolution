...
PSA <- dpp %>% 
  ...
  scale_y_continuous(expand = c(0,0)) + 
  scale_x_continuous(expand = c(0,0)) + 
  labs(x = bquote("Temperature-dependent expression rate (1 +"~italic(ß)[T]*")"), # beta = ALT + 225 
       y = bquote("Feminization threshold ("*italic(theta)[F]*")"), 
       fill = bquote("Frequency of"~italic(F)^I)) + 
...
# Write to output.
pdf(file = paste(simdate, "_paper_figure_v2021_06_15.pdf", sep = ""), width = 7, height = 3.5)
paper_figure
dev.off()

png(file = paste(simdate, "_paper_figure_v2021_06_15.png", sep = ""), width = 7, height = 3.5, units = "in", res = 1000)
paper_figure
dev.off()
