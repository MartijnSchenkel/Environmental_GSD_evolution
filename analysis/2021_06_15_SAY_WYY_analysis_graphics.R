...
dpp <- dpp %>% as_tibble() %>% mutate(fL = str_c("italic(",str_sub(Locus, 1, 1), ")^", str_sub(Locus, 2,2)),
  ...
  mutate(fL = factor(fL, c("italic(F)^I", "italic(Y)^M", "italic(A)^M")))
...
# Save plot
pdf(file = paste(simdate, "_allele_frequencies_v2021_06_15.pdf", sep = ""), height = 7, width = 7)
plot
dev.off()

png(file = paste(simdate, "_allele_frequencies_v2021_06_15.png", sep = ""), height = 7, width = 7, units = "in", res = 1000)
plot
dev.off()
...
pdf(file = paste(simdate, "_Musca_evolution_v2021_06_15.pdf", sep = ""), height = 3.2, width = 4)
plot_combined
dev.off()

png(file = paste(simdate, "_Musca_evolution_v2021_06_15.png", sep = ""), height = 3.2, width = 4, units = "in", res = 1000)
plot_combined
dev.off()
